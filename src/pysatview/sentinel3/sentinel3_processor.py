...
class Sentinel3DataProcessor:
    ...
    def __init__(self, base_dir: Path = pkg_path / "test_data" / "sentinel3"):
    ...
    def download_data(
        ...
        for layer_key in layer_keys:
            try:
                file_paths = self._download_single_layer(
                    layer_key, region, time_range, format_option
                )
                downloaded_files[layer_key] = file_paths
                print(f"✓ Downloaded {layer_key}: {', '.join([str(p) for p in file_paths])}")
    ...
    def _download_single_layer(
        ...
        timestamp = time_range[1].replace(':','')
        filename = f"{timestamp}_TEMP.nc"
        ...
        # Try crop out nan steps
        # try:
        ds = xr.open_dataset(file_path)
        ds = ds.dropna(dim='time', how='all')
        # except Exception as e:
        #     print('Could not crop S3 download')
            
        # Save each time step as a separate file if there are multiple time steps
        save_all_paths = []
        if 'time' in ds.dims: # and ds.sizes['time'] > 1:
            for itime in ds['time']:
                # Get the time for filename
                save_name = pd.to_datetime(itime.values).strftime("%Y%m%d_%H%M%S") + '.nc'
                save_path = self.get_nc_path(satellite, data_type, save_name)
                
                # Check if file already exists
                if save_path.exists():
                    print(f"Already processed: {save_path}")
                    # save_all_paths.append(save_path)
                    continue
            
                # Save this time step as a separate file
                ds_time = ds.sel(time=itime)
                ds_time.to_netcdf(save_path)
                print(f"Saved time step: {save_path}")
                save_all_paths.append(save_path)
    ...
    def process_and_visualize(
        ...
        for layer_key in downloaded_files.keys():
    ...
    def _create_single_plot(
        ...
        if not png_path.exists():
            ...
            if data_type == 'chl':
                im = ax.pcolormesh(ds['lon'], ds['lat'], data, norm=LogNorm(vmin=data.quantile(0.025), vmax=data.quantile(0.975)), cmap=cmap)
            else:
...
class Sentinel3Workflow:
    ...
    def __init__(self, base_dir: Path = pkg_path / "test_data" / "sentinel3"):
        self.processor = Sentinel3DataProcessor(base_dir)
...
class Sentinel3FileMonitor:
    """EUMETView file integrity monitoring and repair service"""
    
    def __init__(self, processor: Sentinel3DataProcessor):
        """
...
def create_file_monitor(base_dir: str = "data/eumetview_sentinel3") -> Sentinel3FileMonitor:
    """
    ...
    """
    processor = Sentinel3DataProcessor(base_dir)
    return Sentinel3FileMonitor(processor)
